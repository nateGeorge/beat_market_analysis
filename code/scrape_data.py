...
def download_vioo_holdings(driver):
    ...
    while True:
        ...
        try:
            ...
            """
            lots of stuff trying to click on 'view holding details button' with no success

            # first need to find button to 'view holding details'
            hd = driver.find_element_by_xpath("//div[contains(text(),'View holding details')]")
            actions = ActionChains(driver)
            # actions.move_to_element(hd).perform()
            driver.execute_script("arguments[0].scrollIntoView();", hd)

            hover = ActionChains(driver).move_to_element(hd)
            hover.perform()

            driver.execute_script("arguments[0].click();", hd)

            link = driver.find_element_by_xpath('/html/body/div[1]/main/div/div/div/div[1]/portfolio/div/div[2]/div[3]/holding-details/div/div/div/div[2]/div/a')

            driver.find_element_by_xpath('/html/body/div[1]/main/div/div/div/div[1]/portfolio/div/div[2]/div[3]/holding-details/div/div/div/div[2]/div/a/div/div[1]').click()


            driver.find_element_by_xpath('/html/body/div[1]/main/div/div/div/div[1]/div[1]/div/div[2]/div/total-returns/div/div[1]/div/div[1]/div[2]/div/button').click()

            el = driver.find_element_by_css_selector('a[data-ng-click="animateTab()"]')
            driver.execute_script("arguments[0].scrollIntoView();", el)
            ActionChains(driver).move_to_element(el).perform()
            driver.execute_script("arguments[0].click();", el)
            """

            # just go straight to element and click it using js...seems to work
            el = driver.find_element_by_xpath('/html/body/div[1]/main/div/div/div/div[1]/portfolio/div/div[2]/div[3]/holding-details/div/div/div/div[1]/div/data-ng-include/div[1]/div/div[2]/div/button')
            driver.execute_script("arguments[0].click();", el)
